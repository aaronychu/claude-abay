...
describe('Feishu: project picker card', () => {
  const sampleProjects: RecentProject[] = [
    {
      projectPath: '/Users/dev/claude-code-abay',
      realPath: '/Users/dev/claude-code-abay',
      projectName: 'claude-code-abay',
      isGit: true,
      repoName: 'claude-code-abay',
      branch: 'main',
  ...
  it('info column has title markdown + notation path markdown', () => {
    ...
    expect(info[0].content).toContain('**claude-code-abay**')
    ...
    expect(info[1].content).toContain('claude-code-abay')
  })
  ...
  it('row button says 选择 with small size and carries per-project value', () => {
    ...
    expect(btn1.value.realPath).toBe('/Users/dev/claude-code-abay')
    expect(btn1.value.projectName).toBe('claude-code-abay')
...
describe('Feishu: card.action.trigger parsing', () => {
  ...
  it('parses pick_project action from event', () => {
    ...
    expect(event.action.value.realPath).toBe('/Users/dev/claude-code-abay')
    expect(event.action.value.projectName).toBe('claude-code-abay')
  })
