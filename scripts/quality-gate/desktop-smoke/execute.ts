...
export async function executeDesktopSmoke(
  ...
  try {
    ...
    const browserSetup = [
      `localStorage.setItem('claude-abay-open-tabs', ${JSON.stringify(JSON.stringify({
        openTabs: [{ sessionId: session.sessionId, title: 'Desktop Smoke', type: 'session' }],
        activeTabId: session.sessionId,
      }))})`,
      runtimeSelection
        ? `localStorage.setItem('claude-abay-session-runtime', ${JSON.stringify(JSON.stringify({
          [session.sessionId]: runtimeSelection,
        }))})`
        : `localStorage.removeItem('claude-abay-session-runtime')`,
    ]
