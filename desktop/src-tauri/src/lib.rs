...
fn window_state_path(app: &AppHandle) -> Option<PathBuf> {
    ...
    // %APPDATA%\com.claude-code-abay.desktop\.
...
fn server_state_path() -> Option<PathBuf> {
    // Lives next to claude-abay/settings.json (CLAUDE_CONFIG_DIR or ~/.claude) so
...
fn setup_system_tray(app: &mut tauri::App) -> tauri::Result<()> {
    let menu = MenuBuilder::new(app)
        .text(TRAY_SHOW_ID, "Show Claude Code A+BAY")
        .separator()
        .text(TRAY_QUIT_ID, "Quit Claude Code A+BAY")
        .build()?;

    let mut tray = TrayIconBuilder::with_id("main-tray")
        .tooltip("Claude Code A+BAY")
        .menu(&menu)
...
/// 解析 claude-abay/settings.json 里的 h5Access.fixedPort。范围必须与
...
fn read_h5_fixed_port() -> Option<u16> {
    let path = claude_config_dir()?.join("claude-abay").join("settings.json");
...
pub fn run() {
    ...
    let builder = builder
        .menu(|app| {
            let about_item =
                MenuItemBuilder::with_id("nav_about", "关于 Claude Code A+BAY").build(app)?;
            let settings_item = MenuItemBuilder::with_id("nav_settings", "设置...")
                .accelerator("CmdOrCtrl+,")
                .build(app)?;

            let app_submenu = SubmenuBuilder::new(app, "Claude Code A+BAY")
                .item(&about_item)
                .separator()
                .item(&settings_item)
                .separator()
