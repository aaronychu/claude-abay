...
Function CcHahaCanSkipLegacyRecovery
  ...
  IfFileExists "$R1\Claude Code A+BAY\app-mode.json" cc_haha_check_default_mode 0
  StrCpy $R0 "1"
  Goto cc_haha_skip_recovery_done

  cc_haha_check_default_mode:
    ClearErrors
    FileOpen $R2 "$R1\Claude Code A+BAY\app-mode.json" r
...
Function CcHahaRecoverLegacy
  ...
  DetailPrint "Checking registered installations for legacy Claude Code A+BAY data..."
  nsExec::ExecToStack '"$SYSDIR\WindowsPowerShell\v1.0\powershell.exe" -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "$PLUGINSDIR\recover-legacy-install-data.ps1" -PerUserInstallDir "$4" -PerMachineInstallDir "$5" -CandidateInstallDir "$9" -UserDataDir "$2\Claude Code A+BAY" -RecoveryRoot "$3\Claude Code A+BAY Data\Recovered" -ProcessName "${PRODUCT_FILENAME}.exe" -ActiveConfigDir "$6" -ActiveConfigManaged "$7" -InstallerIdentitySafety "$8"'
  Pop $0
  Pop $1
FunctionEnd
    ...
    ${Else}
      ...
      ${Else}
        ...
        ${If} $0 != "0"
          ...
          ${EndIf}
          StrCpy $R2 "$1" 360
          MessageBox MB_ICONSTOP|MB_OK "Claude Code A+BAY stopped setup before removing the old version. Reason: $R2$\r$\n$\r$\nClose the app and retry. If the reason mentions an elevated installer, launch setup normally instead of using Run as administrator.$\r$\n$\r$\nClaude Code A+BAY 已在删除旧版本前停止安装。原因：$R2$\r$\n$\r$\n请关闭旧程序后重试；如果原因提到安装器权限过高，请直接双击运行，不要使用“以管理员身份运行”。旧版本和原数据尚未删除。" /SD IDOK
          SetErrorLevel 20
          Quit
        ${EndIf}
        StrCpy $ccHahaRecoveryDone "1"
        DetailPrint "Legacy Claude Code A+BAY data safety check completed"
      ${EndIf}
    ${EndIf}
  ${EndIf}
!macroend
...
!macro CcHahaFindInstallProcess _FILE _RETURN
  ${If} $IsPowerShellAvailable == 0
    ...
    ${EndIf}
  ${Else}
    Delete "$PLUGINSDIR\claude-abay-processes.csv"
    !ifdef INSTALL_MODE_PER_ALL_USERS
      nsExec::Exec '"$CmdPath" /D /C tasklist /FO CSV /NH > "$PLUGINSDIR\claude-abay-processes.csv"'
    !else
      nsExec::Exec '"$CmdPath" /D /C tasklist /FI "USERNAME eq %USERNAME%" /FO CSV /NH > "$PLUGINSDIR\claude-abay-processes.csv"'
    !endif
    Pop ${_RETURN}
    ${If} ${_RETURN} != 0
      ...
      StrCpy ${_RETURN} 0
    ${Else}
      nsExec::Exec '"$SYSDIR\findstr.exe" /I /L /C:"${_FILE}" /C:"claude-sidecar-x86_64-pc-windows-msvc.exe" /C:"claude-sidecar-aarch64-pc-windows-msvc.exe" /C:"claude-sidecar.exe" /C:"OpenConsole.exe" /C:"winpty-agent.exe" /C:"rg.exe" "$PLUGINSDIR\claude-abay-processes.csv"'
      Pop ${_RETURN}
      ${If} ${_RETURN} == 0
