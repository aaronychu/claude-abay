...
param(
  ...
  [string]$UserDataDir = '',
  [string]$RecoveryRoot = '',
  [string]$ProcessName = 'Claude Code A+BAY.exe',
...
function Test-LegacyPortableData {
  ...
  foreach ($childDir in @('Cache', 'EBWebView', 'projects', 'skills', 'plugins', 'cowork_plugins', 'claude-abay')) {
    ...
    foreach ($installDir in $InstallDirs) {
      if (Test-PathMayBeDeleted -InstallDir $installDir -Candidate $active) {
        ...
        if ($ActiveConfigManaged -ne '1') {
          throw "Active CLAUDE_CONFIG_DIR is managed outside Claude Code A+BAY and points inside an application install directory. Move or remove that environment variable before upgrading: $active"
        }
...
function Run-SelfTest {
  $testRoot = Join-Path ([IO.Path]::GetTempPath()) "claude-abay-storage-recovery-$([Guid]::NewGuid().ToString('N'))"
  ...
  try {
    ...
    try {
      ...
      $unmanagedFailed = $_.Exception.Message.Contains('managed outside Claude Code A+BAY')
    }
