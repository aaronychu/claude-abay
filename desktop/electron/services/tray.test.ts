  ...
  it('installs tray handlers that show the app, quit explicitly, and dispose cleanly', async () => {
    ...
    try {
      ...
      const controller = await installTray({
        app: { name: 'Claude Code A+BAY' } as never,
        desktopRoot: root,
        show,
        quit,
      })
      ...
      expect(trayMocks.tray.setToolTip).toHaveBeenCalledWith('Claude Code A+BAY')
      ...
      expect(template.map(item => item.label ?? item.type)).toEqual([
        'Show Claude Code A+BAY',
        'separator',
        'Quit Claude Code A+BAY',
      ])
