...
describe('Electron IPC capabilities', () => {
  ...
  it('validates structured payloads before they reach ipcRenderer.invoke', () => {
    ...
    expect(validateElectronIpcPayload(ELECTRON_IPC_CHANNELS.appSetTheme, 'white')).toBe(true)
    expect(validateElectronIpcPayload(ELECTRON_IPC_CHANNELS.appSetTheme, 'dark')).toBe(true)
    expect(validateElectronIpcPayload(ELECTRON_IPC_CHANNELS.appSetTheme, 'system')).toBe(false)
