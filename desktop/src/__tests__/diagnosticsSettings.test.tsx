...
describe('Settings > Diagnostics tab', () => {
  beforeEach(() => {
    ...
    diagnosticsApiMock.getStatus.mockResolvedValue({
      ...
      cliDiagnosticsPath: '/tmp/claude/claude-abay/diagnostics/cli-diagnostics.jsonl',
      runtimeErrorsPath: '/tmp/claude/claude-abay/diagnostics/runtime-errors.log',
      exportDir: '/tmp/claude/claude-abay/diagnostics/exports',
      retentionDays: 7,
    ...
    diagnosticsApiMock.exportBundle.mockResolvedValue({
      bundle: {
        path: '/tmp/claude/claude-abay/diagnostics/exports/claude-abay-diagnostics.tar.gz',
        fileName: 'claude-abay-diagnostics.tar.gz',
        bytes: 1024,
      },
    })
    ...
    doctorApiMock.report.mockResolvedValue({
      report: {
        ...
        items: [
          {
            id: 'claude-abay-providers',
            label: 'Managed providers',
            kind: 'json',
            scope: 'user',
            path: '~/.claude/claude-abay/providers.json',
            protected: true,
            exists: true,
  ...
  it('shows diagnostics status, actions, and recent events', async () => {
    ...
    expect(screen.getByText('/tmp/claude/claude-abay/diagnostics')).toBeInTheDocument()
  ...
  it('lets a pending rebuild win over an export refresh and always releases its busy state', async () => {
    ...
    await act(async () => {
      exportRequest.resolve({
        bundle: {
          path: '/tmp/claude/claude-abay/diagnostics/exports/race.tar.gz',
  ...
  it('keeps legacy diagnostics usable when the additive local-index endpoint is unavailable', async () => {
    ...
    expect(await screen.findByText('/tmp/claude/claude-abay/diagnostics')).toBeInTheDocument()
  ...
  it('checks findings first and confirms before resetting only safe desktop state', async () => {
    ...
    expect(window.localStorage.getItem('claude-abay-theme')).toBe('claude-abay-theme-value')
    expect(screen.getByText('~/.claude/claude-abay/providers.json')).toBeInTheDocument()
    ...
    expect(screen.getByText(/claude-abay-app-zoom/)).toBeInTheDocument()
    expect(screen.getByText(/claude-abay-ui-zoom/)).toBeInTheDocument()
    ...
    expect(screen.getByText(/Removed keys:.*claude-abay-app-zoom/)).toBeInTheDocument()
  })
  ...
  it('clears an existing Doctor report when the active cwd changes', async () => {
    ...
    fireEvent.click(screen.getByText('Diagnostics'))
    fireEvent.click(await screen.findByRole('button', { name: /Run Doctor/i }))
    expect(await screen.findByText('~/.claude/claude-abay/providers.json')).toBeInTheDocument()
    ...
    await waitFor(() => {
      expect(screen.queryByText('~/.claude/claude-abay/providers.json')).not.toBeInTheDocument()
    })
  ...
  it('preserves reset results and releases reset loading when cwd changes during refresh', async () => {
    ...
    expect(await screen.findByText(/Removed keys:.*claude-abay-app-zoom/)).toBeInTheDocument()
