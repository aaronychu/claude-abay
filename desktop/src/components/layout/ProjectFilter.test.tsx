...
describe('ProjectFilter', () => {
  beforeEach(() => {
    ...
    useSessionStore.setState({
      ...
      availableProjects: [
        ...
        'Users-nanmi-workspace-myself_code-claude-code-abay',
      ],
    })
  })
  ...
  it('renders recent project metadata instead of bare fallback folder names', async () => {
    getRecentProjectsMock.mockResolvedValue({
      projects: [
        {
          projectPath: 'Users-nanmi-workspace-myself_code-claude-code-abay',
          realPath: '/Users/nanmi/workspace/myself_code/claude-code-abay',
          projectName: 'claude-code-abay',
          isGit: true,
          repoName: 'NanmiCoder/claude-abay',
          branch: 'main',
    ...
    await waitFor(() => {
      expect(screen.getByText('NanmiCoder/claude-abay')).toBeInTheDocument()
      expect(screen.getByText('/Users/nanmi/workspace/myself_code/claude-code-abay')).toBeInTheDocument()
      expect(screen.getByText('NanmiCoder/OpenCutSkill')).toBeInTheDocument()
    })

    fireEvent.click(screen.getByRole('button', { name: /NanmiCoder\/claude-abay/i }))

    await waitFor(() => {
      expect(useSessionStore.getState().selectedProjects).toEqual(['Users-nanmi-workspace-myself_code-claude-code-abay'])
    })

    expect(screen.getAllByRole('button', { name: /NanmiCoder\/claude-abay/i })).toHaveLength(2)
  })
})
