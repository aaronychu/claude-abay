  ...
  it('does not duplicate the branch in the selected project chip', async () => {
    vi.mocked(sessionsApi.getRecentProjects).mockResolvedValue({
      projects: [{
        ...
        projectName: 'project',
        repoName: 'aaronychu/OpenCutSkill',
        branch: 'main',
        isGit: true,
    ...
    const trigger = await waitFor(() => screen.getAllByRole('button', { name: /aaronychu\/OpenCutSkill/ })[0])
    expect(trigger).toHaveTextContent('aaronychu/OpenCutSkill')
