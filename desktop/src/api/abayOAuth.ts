// desktop/src/api/abayOAuth.ts

import { api, getBaseUrl } from './client'

export type AbayOAuthStatus =
  | { loggedIn: false }
  | {
      loggedIn: true
...
export const abayOAuthApi = {
  start() {
    return api.post<{ authorizeUrl: string; state: string }>(
      '/api/abay-oauth/start',
      { serverPort: currentServerPort() },
    )
  },

  status() {
    return api.get<AbayOAuthStatus>('/api/abay-oauth')
  },

  logout() {
    return api.delete<{ ok: true }>('/api/abay-oauth')
  },
}
