...
describe('doctorRepair', () => {
  ...
  it('clears only the safe desktop UI storage keys', () => {
    ...
    window.localStorage.setItem('claude-abay-chat-history', 'preserve')
    window.localStorage.setItem('claude-abay-provider-config', 'preserve')
    ...
    expect(window.localStorage.getItem('claude-abay-chat-history')).toBe('preserve')
    expect(window.localStorage.getItem('claude-abay-provider-config')).toBe('preserve')
  })
  ...
  it('checks the server report for the active cwd without clearing desktop state', async () => {
    ...
    expect(window.localStorage.getItem('claude-abay-theme')).toBe('dark')
  })
})
