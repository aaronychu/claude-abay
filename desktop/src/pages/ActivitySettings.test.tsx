...
describe('ActivitySettings', () => {
  ...
  it('renders summary metrics and a GitHub-style trailing heatmap without future days', async () => {
    ...
    expect(screen.getByText('claude-abay')).toBeInTheDocument()
    expect(screen.getByAltText('claude-abay avatar')).toHaveAttribute('src', '/app-icon.png')
    expect(screen.getByAltText('claude-abay avatar')).toHaveClass('scale-[1.28]')
    expect(screen.getByRole('link', { name: 'github.com/aaronychu/claude-abay' })).toHaveAttribute(
      'href',
      'https://github.com/aaronychu/claude-abay',
    )
