...
$installers = @(Get-ChildItem -LiteralPath $resolvedArtifactsDir -File |
  Where-Object { $_.Name -like "Claude-Code-ABAY-*-win-$Arch.exe" })
if ($installers.Count -ne 1) {
  throw "Expected exactly one Windows $Arch installer in $resolvedArtifactsDir, found $($installers.Count)."
}
$installer = $installers[0].FullName

$testRoot = Join-Path ([IO.Path]::GetTempPath()) "claude-abay-installer-smoke-$([Guid]::NewGuid().ToString('N'))"
$installDir = Join-Path $testRoot 'Claude Code A+BAY'
...
$appExe = Join-Path $installDir 'Claude Code A+BAY.exe'
$uninstaller = Join-Path $installDir 'Uninstall Claude Code A+BAY.exe'
...
function Invoke-LegacyRecoveryDiagnostic {
  ...
  $arguments = @(
    ...
    $installDir,
    '-UserDataDir',
    (Join-Path $appData 'Claude Code A+BAY'),
    '-RecoveryRoot',
    (Join-Path $userProfile 'Claude Code A+BAY Data\Recovered'),
    '-ProcessName',
    'Claude Code A+BAY.exe',
...
try {
  ...
  $siblingProbe = Join-Path $siblingDir 'Claude Code A+BAY.exe'
  ...
  Invoke-ProcessHelperExpectExit `
    ...
    -Arguments @(
      ...
      '-ProcessName', 'Claude Code A+BAY.exe',
      '-Action', 'Find',
