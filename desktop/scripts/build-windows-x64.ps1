...
function Get-StagedArtifactName {
  ...
  switch -Regex ($ArtifactName) {
    ...
    '\.msi\.zip\.sig$' { return "Claude-Code-ABAY_${appVersion}_windows_x64_msi.msi.zip.sig" }
    '\.msi\.zip$' { return "Claude-Code-ABAY_${appVersion}_windows_x64_msi.msi.zip" }
    '\.msi\.sig$' { return "Claude-Code-ABAY_${appVersion}_windows_x64_msi.msi.sig" }
    '\.msi$' { return "Claude-Code-ABAY_${appVersion}_windows_x64_msi.msi" }
...
if (-not $env:TAURI_SIGNING_PRIVATE_KEY) {
  $tempConfigPath = Join-Path ([System.IO.Path]::GetTempPath()) 'claude-abay.tauri.local.windows.json'
  $tempConfig = @{
    bundle = @{
