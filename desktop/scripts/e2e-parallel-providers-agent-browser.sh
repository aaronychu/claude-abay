...
ARTIFACT_DIR="${ARTIFACT_DIR:-/tmp/claude-abay-e2e-parallel-providers-$$}"
CONFIG_DIR="${ARTIFACT_DIR}/config"
PROJECT_A="${ARTIFACT_DIR}/project-a"
PROJECT_B="${ARTIFACT_DIR}/project-b"
SESSION_NAME_A="${SESSION_NAME_A:-claude-abay-provider-a-$$}"
SESSION_NAME_B="${SESSION_NAME_B:-claude-abay-provider-b-$$}"
...
mkdir -p "${ARTIFACT_DIR}" "${CONFIG_DIR}/claude-abay" "${PROJECT_A}" "${PROJECT_B}"
...
mkdirSync(join(configDir, 'claude-abay'), { recursive: true })
writeFileSync(join(configDir, 'claude-abay', 'providers.json'), JSON.stringify({
  schemaVersion: 1,
...
setup_browser() {
  ...
  "${ab[@]}" eval "
    localStorage.setItem('claude-abay-open-tabs', JSON.stringify({
      openTabs: [{ sessionId: '${session_id}', title: '${title}', type: 'session' }],
      activeTabId: '${session_id}',
    }));
    localStorage.setItem('claude-abay-session-runtime', JSON.stringify({
