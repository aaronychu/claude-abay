...
#
# Installs the unsigned macOS build of Claude Code A+BAY from a DMG that sits
...
#   bash install-macos-unsigned.sh /path/to/Claude-Code-ABAY-0.4.0-mac-arm64.dmg

APP_NAME="Claude Code A+BAY.app"
...
find_dmg() {
  ...
  shopt -s nullglob
  local matches=("${dir}"/Claude-Code-ABAY-*-mac-*.dmg)
  shopt -u nullglob
...
main() {
  ...
  if [ -z "$dmg" ]; then
    if ! dmg="$(find_dmg "$base_dir")"; then
      echo "No Claude Code A+BAY macOS DMG found next to this script."
      echo "Download the DMG into the same folder as this script, then run it again."
      echo "Usage: bash install-macos-unsigned.sh /path/to/Claude-Code-ABAY-0.4.0-mac-arm64.dmg"
      exit 1
    fi
  fi
  ...
  osascript -e 'quit app "Claude Code A+BAY"' >/dev/null 2>&1 || true

  if [ -d "$APP_PATH" ]; then
    local backup
    backup="${HOME}/.Trash/Claude Code A+BAY.$(date +%Y%m%d%H%M%S).app"
  ...
  echo "Opening Claude Code A+BAY..."
  open "$APP_PATH"
}
