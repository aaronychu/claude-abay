...
export function getTraceStorageDir(): string {
  return join(getClaudeConfigHomeDir(), 'claude-abay', 'traces')
}
...
function currentTraceScopeContext(): TraceScopeContext {
  ...
  return {
    scope,
    storageDir: join(scope, 'claude-abay', 'traces'),
    target: {
      path: join(scope, 'claude-abay', 'db', 'trace-index-v1.sqlite'),
      scope,
    },
  }
}
...
function getManagedSettingsPath(scope = getClaudeConfigHomeDir()): string {
  return join(scope, 'claude-abay', 'settings.json')
}
