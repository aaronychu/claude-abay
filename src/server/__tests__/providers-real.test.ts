 ...
 * 验证添加、激活、claude-abay/settings.json 同步是否正确
 * (provider env 写到 ~/.claude/claude-abay/settings.json，不污染原版 settings.json)
 */
...
describe('Real Provider Configs', () => {
  ...
  test('添加 MiniMax Provider 并激活 — 写入 claude-abay/settings.json', async () => {
    ...
    console.log('✅ Provider 写入 claude-abay/settings.json，原版 settings.json 未被污染')
  })

  test('切换 Provider — 更新 claude-abay/settings.json', async () => {
    const minimax = await service.addProvider({
      presetId: 'minimax',
      name: 'MiniMax',
      baseUrl: 'https://api.minimaxi.com/anthropic',
    ...
    console.log('✅ 切换 Provider 成功，claude-abay/settings.json 更新正确')
  })

  test('claude-abay/settings.json 保留已有字段', async () => {
    // 预写一个有内容的 claude-abay/settings.json（模拟用户已有配置）
    await fs.mkdir(path.join(tmpDir, 'claude-abay'), { recursive: true })
    await fs.writeFile(
      path.join(tmpDir, 'claude-abay', 'settings.json'),
      JSON.stringify({
    ...
    console.log('✅ claude-abay/settings.json 已有字段全部保留')
  })
  ...
  test('providers.json 和 claude-abay/settings.json 独立于 settings.json', async () => {
