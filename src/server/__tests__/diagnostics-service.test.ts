...
beforeEach(async () => {
  tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'claude-abay-diagnostics-test-'))
...
describe('DiagnosticsService', () => {
  ...
  test('defaults an unclassified event to info, not error', async () => {
    ...
    await expect(
      fs.readFile(path.join(tmpDir, 'claude-abay', 'diagnostics', 'runtime-errors.log'), 'utf-8'),
    ).rejects.toThrow()
  })
  ...
  test('exports a single diagnostics tarball without provider secrets', async () => {
    ...
    await fs.writeFile(
      path.join(tmpDir, 'claude-abay', 'diagnostics', 'cli-diagnostics.jsonl'),
...
describe('diagnostics API', () => {
  test('returns status, events, export path, and supports clearing logs', async () => {
    ...
    expect(status.logDir).toContain(path.join('claude-abay', 'diagnostics'))
