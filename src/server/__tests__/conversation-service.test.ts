...
describe('ConversationService', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'claude-abay-conversation-service-'))
  ...
  test('keeps inherited provider env when no desktop provider config exists', async () => {
    ...
    const env = (await service.buildChildEnv('D:\\workspace\\code\\myself_code\\claude-abay')) as Record<string, string>

    expect(env.ANTHROPIC_AUTH_TOKEN).toBe('test-token')
    expect(env.ANTHROPIC_BASE_URL).toBe('https://example.invalid/anthropic')
    expect(env.ANTHROPIC_MODEL).toBe('test-model')
    expect(env.CLAUDE_CODE_DIAGNOSTICS_FILE).toBe(path.join(tmpDir, 'claude-abay', 'diagnostics', 'cli-diagnostics.jsonl'))
  ...
  test('strips inherited provider env when desktop provider config exists', async () => {
    const abayDir = path.join(tmpDir, 'claude-abay')
    await fs.mkdir(abayDir, { recursive: true })
    await fs.writeFile(
      path.join(abayDir, 'providers.json'),
  ...
  test('buildChildEnv injects CLAUDE_CODE_OAUTH_TOKEN when official mode + A+BAY oauth token exists', async () => {
    const abayDir = path.join(tmpDir, 'claude-abay')
    await fs.mkdir(abayDir, { recursive: true })
    await fs.writeFile(
      path.join(abayDir, 'settings.json'),
      JSON.stringify({ env: {} }),
      'utf-8',
    )

    const { abayOAuthService } = await import('../services/abayOAuthService.js')
    await abayOAuthService.saveTokens({
      accessToken: 'abay-fresh-token',
      refreshToken: 'abay-refresh-xxx',
    ...
    expect(env.CLAUDE_CODE_OAUTH_TOKEN).toBe('abay-fresh-token')
  })
  ...
  test('buildChildEnv does NOT inject CLAUDE_CODE_OAUTH_TOKEN when not official mode', async () => {
    ...
    const { abayOAuthService } = await import('../services/abayOAuthService.js')
    await abayOAuthService.saveTokens({
      accessToken: 'abay-token-should-not-be-used',
      refreshToken: null,
      expiresAt: null,
      scopes: [],
  ...
  test('buildChildEnv injects desktop Computer Use host bundle id for sdk sessions', async () => {
    ...
    expect(env.CLAUDE_ABAY_COMPUTER_USE_HOST_BUNDLE_ID).toBe(
      'com.claude-code-abay.desktop',
    )
    expect(env.CLAUDE_ABAY_DESKTOP_SERVER_URL).toBe('http://127.0.0.1:3456')
  ...
  test('uses bun entrypoint fallback on Windows dev mode', () => {
    ...
    if (process.platform === 'win32') {
      ...
      expect(args[0]).toContain(path.join('bin', 'claude-abay'))
    }
  })
  ...
  test('buildChildEnv asks desktop SDK sessions to wait briefly for MCP tools', async () => {
    ...
    expect(env.CLAUDE_ABAY_DESKTOP_AWAIT_MCP).toBe('1')
    expect(env.CLAUDE_ABAY_DESKTOP_AWAIT_MCP_TIMEOUT_MS).toBe('5000')
  })
