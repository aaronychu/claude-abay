...
describe('background task notification persistence', () => {
  ...
  beforeEach(async () => {
    ...
    configDir = await fs.mkdtemp(path.join(os.tmpdir(), 'claude-abay-task-notification-'))
  ...
  it('restores a terminal Agent notification that was not forwarded into transcript history', async () => {
    ...
    expect(await fs.readFile(transcriptPath, 'utf8')).toContain('"type":"claude-abay-task-notification"')
  })
