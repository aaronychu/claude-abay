...
beforeEach(async () => {
  ...
  await fs.mkdir(path.join(configDir, 'claude-abay'), { recursive: true })
  ...
  await fs.writeFile(
    path.join(configDir, 'claude-abay', 'providers.json'),
...
describe('DoctorService', () => {
  ...
  test('reports schema-invalid managed providers without exposing parsed contents', async () => {
    ...
    await fs.mkdir(path.join(schemaConfigDir, 'claude-abay'), { recursive: true })
    await fs.writeFile(
      path.join(schemaConfigDir, 'claude-abay', 'providers.json'),
      JSON.stringify({ activeId: null, providers: [{ id: 'provider-1' }] }),
      'utf-8',
    )
    ...
    const report = await service.getReport()
    const providers = report.items.find((item) => item.id === 'claude-abay-providers')
