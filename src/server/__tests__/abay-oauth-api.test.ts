/**
 * Integration tests for /api/abay-oauth/* endpoints.
 */
...
async function setup() {
  tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'abay-oauth-api-test-'))
...
describe('POST /api/abay-oauth/start', () => {
  beforeEach(setup)
  afterEach(teardown)

  test('returns authorize URL with PKCE challenge', async () => {
    const { req, url, segments } = buildReq('POST', '/api/abay-oauth/start', {
      serverPort: 54321,
    })
    const res = await handleAbayOAuthApi(req, url, segments)
...
describe('GET /api/abay-oauth/status', () => {
  beforeEach(setup)
  afterEach(teardown)

  test('returns loggedIn=false when no token file', async () => {
    const { req, url, segments } = buildReq('GET', '/api/abay-oauth/status')
    const res = await handleAbayOAuthApi(req, url, segments)
    expect(res.status).toBe(200)
  ...
  test('returns loggedIn=false when stored token is expired and refresh fails', async () => {
    ...
    abayOAuthService.setRefreshFn(async () => {
...
describe('DELETE /api/abay-oauth', () => {
  beforeEach(setup)
  afterEach(teardown)
  ...
  test('clears token file', async () => {
    ...
    const { req, url, segments } = buildReq('DELETE', '/api/abay-oauth')
    const res = await handleAbayOAuthApi(req, url, segments)
    expect(res.status).toBe(200)
    expect(await abayOAuthService.loadTokens()).toBeNull()
  })
})
