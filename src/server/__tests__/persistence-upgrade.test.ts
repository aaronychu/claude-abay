...
describe('persistent storage upgrade migrations', () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'claude-abay-persistence-'))
  ...
  test('migrates legacy providers index and writes a backup before changing it', async () => {
    const ccHahaDir = path.join(tempDir, 'claude-abay')
    ...
    expect(report.migratedEntries).toContain('claude-abay/providers.json')
  ...
  test('imports legacy root providers config into claude-abay storage without deleting the source', async () => {
    await fs.writeFile(
    ...
    expect(report.failures).toEqual([])
    expect(report.migratedEntries).toContain('providers.json -> claude-abay/providers.json')
    expect(report.migratedEntries).toContain('providers.json -> claude-abay/settings.json')
    expect(JSON.parse(await fs.readFile(path.join(tempDir, 'providers.json'), 'utf-8'))).toMatchObject({
      version: 1,
      activeModel: 'legacy-sonnet',
    })

    const migrated = JSON.parse(await fs.readFile(path.join(tempDir, 'claude-abay', 'providers.json'), 'utf-8')) as {
      activeId?: string | null
      providerOrder?: string[]
      providers?: Array<{
        id?: string
    ...
    const managedSettings = JSON.parse(await fs.readFile(path.join(tempDir, 'claude-abay', 'settings.json'), 'utf-8')) as {
  ...
  test('does not overwrite current claude-abay provider storage with a legacy root config', async () => {
    const ccHahaDir = path.join(tempDir, 'claude-abay')
    await fs.mkdir(ccHahaDir, { recursive: true })
    await fs.writeFile(
      path.join(tempDir, 'providers.json'),
    ...
    expect(report.failures).toEqual([])
    expect(report.migratedEntries).not.toContain('providers.json -> claude-abay/providers.json')
  ...
  test('quarantines malformed managed settings instead of blocking startup', async () => {
    ...
    expect(report.failures).toEqual([])
    expect(report.migratedEntries).toContain('claude-abay/settings.json')
