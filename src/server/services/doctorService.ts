...
export class DoctorService {
  ...
  private async buildTargets(): Promise<DoctorTarget[]> {
    const targets: DoctorTarget[] = [
      ...
      this.jsonTarget(
        'claude-abay-providers',
        'Managed providers',
        'user',
        path.join(this.configDir, 'claude-abay', 'providers.json'),
      ),
      this.jsonTarget(
        'claude-abay-settings',
        'Managed provider settings',
        'user',
        path.join(this.configDir, 'claude-abay', 'settings.json'),
      ),
      ...
      this.jsonTarget('oauth', 'OAuth tokens', 'user', path.join(this.configDir, 'claude-abay', 'oauth.json')),
      this.jsonTarget(
        'openai-oauth',
        'OpenAI OAuth tokens',
        'user',
        path.join(this.configDir, 'claude-abay', 'openai-oauth.json'),
      ),
      this.jsonTarget(
        'grok-oauth',
        'Grok OAuth tokens',
        'user',
        path.join(this.configDir, 'claude-abay', 'grok-oauth.json'),
      ),
    ]
  ...
  private async inspectJsonTarget(target: DoctorTarget): Promise<DoctorReportItem> {
    ...
    try {
      ...
      if (target.id === 'claude-abay-providers') {
