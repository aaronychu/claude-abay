...
export class DiagnosticsService {
  ...
  getLogDir(): string {
    return path.join(this.getConfigDir(), 'claude-abay', 'diagnostics')
  }
  ...
  async recordEvent(input: DiagnosticEventInput): Promise<void> {
    ...
    // ~/.claude/claude-abay/diagnostics. Tests that genuinely exercise diagnostics
  ...
  async exportBundle(): Promise<DiagnosticsExportResult> {
    ...
    const fileName = `claude-abay-diagnostics-${timestamp}.tar.gz`
  ...
  private buildReadme(): string {
    return [
      'claude-abay diagnostics bundle',
      '',
  ...
  private buildRecentErrorsSummary(events: DiagnosticEvent[]): string {
    ...
    const lines = [
      '# claude-abay recent diagnostics',
      '',
  ...
  private async buildProvidersSummary(): Promise<Record<string, unknown>> {
    const providerPath = path.join(this.getConfigDir(), 'claude-abay', 'providers.json')
    try {
