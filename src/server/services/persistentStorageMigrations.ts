...
async function migrateLegacyRootProviders(
  ...
  try {
    ...
    if (errnoCode(error) !== 'ENOENT') {
      report.failures.push(`claude-abay/providers.json: ${error instanceof Error ? error.message : String(error)}`)
      return
    }
  }
  ...
  try {
    ...
    report.migratedEntries.push('providers.json -> claude-abay/providers.json')
    ...
    if (managedSettings) {
      ...
      report.migratedEntries.push('providers.json -> claude-abay/settings.json')
    }
  } catch (error) {
    if (error instanceof SyntaxError) {
...
async function runPersistentStorageMigrations(configDir: string): Promise<MigrationReport> {
  ...
  await migrateJsonEntry(
    ...
    'claude-abay/providers.json',
    report,
    migrateProvidersIndex,
  )
  await migrateJsonEntry(
    path.join(ccHahaDir, 'settings.json'),
    'claude-abay/settings.json',
    report,
