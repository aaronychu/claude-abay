...
async function createTempDir(label: string): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), `claude-abay-${label}-`))
...
describe('local index coordinator', () => {
  it('enters a busy cooldown after the first indexed read failure', async () => {
    ...
    const coordinator = createLocalIndexCoordinator({
      ...
      resolveDatabasePath: () => '/tmp/config/claude-abay/db/index-v1.sqlite',
  ...
  it('stops serving scope A before reopening the runtime index for scope B', async () => {
    ...
    let activePath = '/tmp/config-a/claude-abay/db/index-v1.sqlite'
    let opens = 0
    let closes = 0
    const indexes = new Map([
      [activePath, createFakeIndex([candidate(1, activeScope)])],
      ['/tmp/config-b/claude-abay/db/index-v1.sqlite', createFakeIndex([
    ...
    activePath = '/tmp/config-b/claude-abay/db/index-v1.sqlite'
  ...
  it('converges external append, create, and delete events through the serial writer pump', async () => {
    ...
    const databasePath = join(configDir, 'claude-abay', 'db', 'index-v1.sqlite')
  ...
  it('auto-recovers a corrupt header once and requires explicit rebuild for a future schema', async () => {
    ...
    expect((await readdir(join(configDir, 'claude-abay', 'db', 'backups'))).length).toBe(1)
    ...
    expect((await readdir(join(configDir, 'claude-abay', 'db', 'backups'))).length).toBe(2)
    expect(await Bun.file(source.path).text()).toBe(sourceBefore)
  ...
  it('resolves a fresh stopped-scope path and rebuilds only that derived database family', async () => {
    ...
    const firstPath = join(firstScope, 'claude-abay', 'db', 'index-v1.sqlite')
    const secondPath = join(secondScope, 'claude-abay', 'db', 'index-v1.sqlite')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const databaseDir = join(configDir, 'claude-abay', 'db')
