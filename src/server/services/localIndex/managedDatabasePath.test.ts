...
describe('managed SQLite database paths', () => {
  test.each([
    ...
    async (_kind, filename, openDatabase) => {
      ...
      await fs.mkdir(path.join(scope, 'claude-abay'), { recursive: true })
      await fs.mkdir(outside)
      await fs.symlink(outside, path.join(scope, 'claude-abay', 'db'))
      const databasePath = path.join(scope, 'claude-abay', 'db', filename)
  ...
  test('allows the configured trust root itself to be a symlink', async () => {
    ...
    const databasePath = path.join(scope, 'claude-abay', 'db', 'index-v1.sqlite')
  ...
  test('rejects an existing database-file symlink', async () => {
    ...
    const scope = path.join(root, 'config')
    const databaseDir = path.join(scope, 'claude-abay', 'db')
