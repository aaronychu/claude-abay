...
async function tempFile(contents: string | Buffer): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'claude-abay-source-fingerprint-'))
  tempDirs.push(root)
...
describe('source fingerprint transitions', () => {
  ...
  it('round-trips a stable versioned fingerprint through the single prefix_hash field', async () => {
    ...
    expect(stored.startsWith('claude-abay-source-fingerprint:v2:')).toBe(true)
    ...
    expect(deserializeSourceFingerprint('claude-abay-source-fingerprint:v2:not-json')).toBeNull()
  })
})
