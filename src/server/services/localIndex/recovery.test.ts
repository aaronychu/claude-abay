...
async function createTempDir(): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'claude-abay-local-index-recovery-'))
...
describe('local index recovery', () => {
  test('moves only the fixed derived database family into the managed backup directory', async () => {
    ...
    const databasePath = join(scope, 'claude-abay', 'db', 'index-v1.sqlite')
    ...
    expect(relative(join(scope, 'claude-abay', 'db', 'backups'), result.backupPath))
      .not.toStartWith('..')
  ...
  test.each(['claude-abay', 'db', 'backups'] as const)(
  ...
  test('rolls back a partial family move and retains no completed backup', async () => {
    ...
    const backupsRoot = join(scope, 'claude-abay', 'db', 'backups')
  ...
  test('keeps only the newest three bounded derived backups', async () => {
    ...
    const backups = await readdir(join(scope, 'claude-abay', 'db', 'backups'))
