...
const DEFAULT_PROFILE_SUBTITLE = 'github.com/aaronychu/claude-abay'
...
export class DesktopUiPreferencesService {
  ...
  private getPreferencesPath(): string {
    return path.join(this.getConfigDir(), 'claude-abay', 'desktop-ui.json')
  }

  private getProfileDir(): string {
    return path.join(this.getConfigDir(), 'claude-abay', 'profile')
  }
  ...
  private getProfileAvatarPath(avatarFile: string): string {
    ...
    return path.join(this.getConfigDir(), 'claude-abay', normalized)
  }
  ...
  async readPreferences(): Promise<DesktopUiPreferencesReadResult> {
    ...
    const preferences = await readRecoverableJsonFile({
      filePath,
      label: 'claude-abay desktop UI preferences',
