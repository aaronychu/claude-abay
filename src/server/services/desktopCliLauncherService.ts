...
const DESKTOP_CLI_NAME = 'claude-abay'
const DESKTOP_CLI_WINDOWS_LEGACY_EXE = `${DESKTOP_CLI_NAME}.exe`
const PATH_BLOCK_START = '# >>> Claude Code A+BAY PATH >>>'
const PATH_BLOCK_END = '# <<< Claude Code A+BAY PATH <<<'
...
if [[ ! -x "$SIDECAR" ]]; then
  echo "claude-abay launcher could not find bundled sidecar: $SIDECAR" >&2
  exit 127
fi
...
export function buildWindowsLauncherWrapper(sourcePath: string) {
  ...
  return [
    ...
    '  echo claude-abay launcher could not find bundled sidecar: %SIDECAR% 1>&2',
    '  exit /b 127',
    ')',
