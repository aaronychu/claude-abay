...
```bash
claude-abay --no-computer-use
CLAUDE_COMPUTER_USE_ENABLED=0 claude-abay
```

You can also write the global config file at `~/.claude/claude-abay/computer-use-config.json`:
