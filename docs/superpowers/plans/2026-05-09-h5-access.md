...
describe('H5AccessService', () => {
  ...
  it('preserves unrelated managed settings fields', async () => {
    ...
    const saved = JSON.parse(await fs.readFile(path.join(tmpDir, 'claude-abay', 'settings.json'), 'utf-8'))
...
- Persist under `claude-abay/settings.json` in an `h5Access` object.
...
```ts
const H5_SERVER_URL_KEY = 'claude-abay-h5-server-url'
const H5_TOKEN_KEY = 'claude-abay-h5-token'
```
...
- Ambiguity resolved: H5 config is stored in `~/.claude/claude-abay/settings.json`, not the shared `~/.claude/settings.json`.
